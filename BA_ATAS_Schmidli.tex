...
Der Tracker verfügt über ein Display, Taste, GPS Modul, Kommunikationsmodul und ein Lautsprecher. Der Tracker überträgt folgende Daten an den Gateway.
\begin{itemize}
	\item Hat der Alpinist ein Problem (Notrufschalter) 
	\item Position des Trackers
\end{itemize}
...
\subsection{Sigfox}
Sigfox ist eine LPWAN Lösung welche vom Netzwerk Design eher an ein herkömmliches Mobilfunknetz erinnert.
...
\begin{itemize}
	\item Sigfox ist eher ein geschlossenes System. Wurde das Gebiet mit Sigfox erschlossen, kann das Netzwerk gegen Bezahlung mitbenutzt werden. Der Aufbau eines eigenen Empfängers scheint nicht möglich. Zum jetzigen Zeitpunkt Stand 15.1.2018 wurde die Schweiz von Sigfox noch nicht erschlossen.
...
NB-IOT (Narrowband - Internet of Things) und LTE Cat M1 wurden als Konkurrenz zu den Populären Lösungen LoRa und Sigfox entwickelt. Auf diese Netzwerke wird hier vorerst nicht eingegangen. Die Standards wurden erst kürzlich von der Industrie aufgegriffen und werde langsam eingeführt. Die Swisscom plant erste Tests ende 2017. Der Rollout für die Nutzer soll 2018 beginnen \cite{swisscomnbiot}.

\subsection{Entscheid}
Sigfox scheidet leider als mögliche Technolgie aus. Die Angaben zur Reichweite sind sehr beeindruckend. Die Beschränkung in der Anzahl der Downlink und Uplink Nachrichten macht es leider für das ATAS System unbrauchbar. Mit nur 140 Nachrichten kann die Position des Trackers nicht sauber ermittelt werden.\\[0.3cm]
...
LoRa unterstützt zwei Modulationsarten, das simple Frequency Shift Key (FSK) und das Chirp Spread Spectrum (CSS) Verfahren. In dieser Arbeit setzen wir den Fokus auf CSS.\\ \bigskip
...
\end{figure}
\noindent
Wie im Bild ersichtlich erhöht sich die Frequenz des Signals. Bei einer Erhöhung der Frequenz reden wir von einem Upchrip, bei einer Reduktion von einem Downchirp. Das Chirp Signal wird anschließend verwendet um die Nutzdaten zu kodieren.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{img/spreadingfactors.png}
	\caption[Wasserfall Darstellung: Spreadingfactors 7-12]
	{Wasserfall Darstellung: Spreadingfactors 7-12}
\end{figure}
\noindent
Wie in der Grafik ersichtlich ist die Sweep Rate bei SF12 viel langsamer als bei SF7. Pro Schritt zwischen den SF (SF7 zu SF8) halbiert sich der Datendurchsatz. Das Senden einer Nachricht dauert länger ist aber einfacher zu dekodieren. Das Signal wird weniger anfällig gegenüber Störungen und kann damit über eine größere Distanz übertragen werden. Müssen größere Datenmengen oder in einem kleineren Zeitintervall versendet werden, bietet es sich an einen kleineren Spreadingfactor zu wählen (7-9).  Sollen große Distanzen überwunden werden, oder gibt es in der Region viele Störsignale, sollte ein hoher Spreadinfactor (10-12) verwendet werden.

\subsubsection{Vorteile}
\begin{enumerate}
	\item CSS benutzt die gesamte Bandbreite bspw. 125kHz um ein Signal zu übertragen. Diese Eigenschaft macht es unempfindlich gegenüber 'Channel-Noise'. Channel Noise beschreibt ein Signal auf einer bestimmten Frequenz.
	\item Nicht betroffen von \gls{Multipathing}
	\item Nicht betroffen vom Doppler Effekt.
\end{enumerate} 
...
\end{tabularx} 
Diese Frequenzbänder gelten für den Einsatz in Kombination mit LoRaWan.

%TODO EXAMPLE

\subsection{Airtime}
Die Airtime definiert, wie lange es dauert ein Signal vom LoRa Endgerät zum Gateway zu übertragen. Die Airtime ist von mehreren Faktoren abhängig
\begin{itemize}
	\item Nutzdatengrösse (Payload), Anzahl Bytes
...
Nachfolgend wird festgestellt, wie schnell wir die Daten per LoRa übertragen können. 
...
\end{figure}
Vielott steht für Abeckung. Weisse Stellen bedeutet es existiert keine LoRaWan Abdeckung. Wie in der Grafik ersichtlich, sind die Bergregionen noch nicht erschlossen. Das Netzwerk der Swisscom kann somit für das ATAS System nicht verwendet werden.

\subsection{Coding Rate}
Die Coding Rate definiert die Error Kodierungsrate. Eine höhere Kodierungsrate ermöglicht es das Signal auch bei hohen Störungen (Noise) zu rekonstruieren. Ein erhöhen der Coding Rate verursacht eine größere Menge an Daten $\rightarrow$ generiert mehr Overhead. Mögliche Werte sind 4/5 und 4/8. Um ein möglichst gutes Signal zu erhalten, soll im ATAS System die Kodierungsrate auf 4/8 festgelegt werden.
...
\newpage
\section{MQTT}
Durch die Wahl vom TTN Netzwerk wurde die Datenübertragungsart MQTT quasi aufgezwungen. Wird der TTN Handler nicht selber verwaltet, ist der Zugriff auf die Daten (API) nur per MQTT möglich  \cite{amqp}. Die Unterstützung von AMQP ist geplant aber noch nicht verfügbar. Das Protokoll MQTT war dem Projektteam bereits bekannt. Während dem Studium konnten bereits positive Erfahrungen mit MQTT gesammelt werden. Das Protokoll ist einfach zu verstehen und hat sich sehr einfach in das ATAS System eingliedern lassen. Die fehlende Websocket Unterstützung konnte mit wenig Aufwand über den Dienst 'CloudMQTT' realisiert werden.
...
Die aktuell beschriebene Komponente wird als grauee Box dargestellt.
\begin{figure}[H]
	\centering
...
\newpage
\section{Broker}
Die Aufgabe des Brokers umfasst das Sammeln der Daten und deren Bereitstellung an die ATAS Applikationen ATAS-Web und ATAS-Service. Der Broker besteht aus zwei Komponenten. Die Komponenten wurde nicht selbstständig entwickelt. Für das ATAS Projekt wurden bereits existierende Dienste verwendet. Die Dienste heissen \textbf{The Things Network (TTN)} und \textbf{CloudMQTT} verwendet. Was genau für Funktionen die einzelnen Dienste bieten wird in den nachfolgenden Abschnitten erklärt.

\subsection{The Things Network (TTN)}
The Things Network kurz TTN ist eine LoRaWan Plattform, welche LoRaWan Endgeräte mit Applikationen verbindet. TTN sendet Daten via Gateways an die Tracker. Über die Gateway empfängt das TTN Daten von den Trackern. Das TTN bietet einen Schnittstelle dem MQTT Broker um auf die Daten zuzugreifen.\\[0.3cm]
\textbf{Diagramm}
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=0.9\textwidth]{img/atasweb/atas-web-node-open.png}
...
\subsubsection{LMIC}
Die LMIC Bibliothek hat den LoRaWan Stack implementiert und wird für die korrekte Kommunikation via LoRa Modul benötigt. Zum Zeitpunkt dieser Arbeit gibt es keine für ESP32 portierte LMIC Bibliothek. Für die LoRaWan Kommunikation wurde daher auf eine bestehende Arduino Library (Arduino-LMIC) zurückgegriffen \cite{ArduinoLMIC}. Der Funktionsumfang bleibt der selbe. Den Kommunikationsablauf selbständig zu programmieren hätte den Umfang dieser Arbeit gesprengt. Einige Entwickler berichten von Timing Problemen während der Kommunikation. 
\begin{figure}[H]
	\centering
\includegraphics[width=0.8\textwidth]{img/prototype/lmic_issue.png}
	\caption[Fehlerhafter Bildaufbau Display]
	{Fehlerhafter Bildaufbau Display}
\end{figure}
Bis jetzt konnten beim ATAS Projekt keine Probleme mit der eingesetzten Library festgestellt werden. Die Arduino Library fügt sich problemlos in die ESP-IDF Umgebung ein.
...
\newpage
\subsection{Herausforderung Timing}
...
Die Resultate mit dem neuen Prototypen sind zufriedenstellend. Das System konnte optimiert werden. Der Prototyp 2 funktioniert einwandfrei. Dem nächsten Schritt, dem finalen Prototypen steht somit nichts mehr im Weg.
...
Abgeleitet aus der vorhergehenden Definitionen ergibt sich die folgende Aufgabenstellung im Bereich Testing.
\begin{itemize}
	\item
