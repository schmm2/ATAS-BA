...
\textsc{\Large Mobile Computing Vertiefung}\\[0.5cm]
...
\item Auf den Einsatz eines Betriebssystems auf dem Tracker Gerät soll verzichtet werden.
\end{itemize}
...
Um die nachfolgenden Kapitel zu verstehen, ist es sehr wichtig, sich mit der im Projekt 2 erstellten Systemarchitektur und der Terminologie vertraut zu machen. Die Architektur wurde im Vergleich zum Projekt 2 um einige Komponenten ergänzt. Der Link zur Projektarbeit kann im Anhang gefunden werden.\\[0.3cm]
...
\begin{itemize}
	...
	\item
		Es können Gefahrenzonen hinterlegt werden.
	\item 
		Zeigt an, ob sich ein Tracker in einer Gefahrenzone aufhält oder ein Notrufsignal abgesetzt hat.	
\end{itemize}

\subsection{Logik}
Die Logik umfasst Software welche berechnet, ob sich ein Tracker in einer Gefahrenzone aufhält. Wenn sich der Tracker in einer Gefahrenzone aufhält, sendet das System einen Alarm an den Tracker. Das Signal gelangt via Datenhandler und Gateway zum Tracker.
...
\begin{itemize}
	\item Bis heute (15.1.2018) wurde die Schweiz von Sigfox nicht erschlossen.
...
NB-IOT \& LTE Cat M1 scheinen eine potente Mobilfunklösung für den IoT Bereich zu sein. Die Zukunft wird zeigen, ob sich die Technologien gegen die bestehende Konkurrenz behaupten können.\\[0.3cm]
...
Für die Schweiz und in vielen Teilen Europas gilt für die Nutzung von LoRa die Regelung ERC/REC 70-03 \cite{regulations}. Zusammengestellt wurden die Regulierungen von der CEPT (European Conference of Postal and Telecommunications). Die Schweiz ist Mitglied dieser Organisation. Die Standardisierung selbst hat die Organisation ETSI (EN300.220) übernommen. Das Dokument beschreibt die verwendbaren Frequenzen, die maximale Übertragungsleistung (dbm) sowie den Duty Cycle.\\[0.3cm]
Die Regulierungen wurden in dieser Projektarbeit eingehalten. Die maximale Sendeleistung von 25mW resp. 14dbm wurde während den Tests nicht überschritten.
...
\newpage
\section{LoRaWan}
LoRaWan ist ein Protokoll welches auf LoRa aufbaut. Das Protokoll definiert die Struktur der gesendeten Daten.
...
\textbf{Die End Nodes} sammeln Daten über Sensoren. Das können ganz unterschiedliche Sachen sein, wie zum Beispiel die Feuchtigkeit in einem Raum. Die End Nodes senden Daten an die Empfänger (Gateways, Concentrator) via LoRa. Die Daten werden hier, nicht wie in einem Mobilfunknetz an nur eine Antenne, sonder an alle Empfänger gesendet.\\[0.3cm]
...
\begin{enumerate}
	...
	\item Das LoRaWan Netzwerk prüft die Angaben
...
Sowohl die eingesetzten Endgeräte wie auch der Gateway unterstützen Downlinks, damit gibt es keine technische Hürde der den Einsatz von OTAA verhindern würde. Aus diesem Grund wurde während dem Projekt die Verbindungsmethodik von ABP auf OTAA umgestellt. Damit konnte die Sicherheit des Systems verbessert werden.
...
Das Endgerät kann immer Senden. Für den Empfang der Daten gibt es bestimmte Regeln. Nach dem Senden öffnet das Endgerät zwei sogenannte Download Receive Fenster. Während dieser Zeit können Daten Empfangen werden. 
...
\end{figure}
\noindent
Werden Daten ausserhalb dieses Zeitfensters geschickt, werden diese nie vom Endgerät empfangen. Alle Geräte der Klasse B und C haben die Funktionalität von Klasse A implementiert. 
...
Ist die Adaptive Data Rate aktiv (ADR), entscheidet das LoRaWan Endgerät selbständig ob und wie die Kommunikation optimiert werden soll. Gemäss TTN \cite{ADRTTN} sollte ADR nur für statische Nodes oder für mobile Nodes zur Erkennung von Stops (keine Bewegung) verwendet werden. Gemäss TTN ist ADR also nicht unbedingt für die ATAS-Tracker geeignet. Die Tracker sind ja meistens in Bewegung bspw. durch Laufen, Ski fahren usw.\\[0.3cm]
Sobald ein LoRaWan Endgerät dem Netzwerk mitteilt, dass es ADR verwenden möchte, beginnt das TTN Daten aufzuzeichnen. Die letzten 20 Messdaten die der Node gesendet hat werden analysiert. Ist der Node nahe dem Gateway wird der Spreadingfactor (SF) reduziert bspw. von SF10 auf SF7. Damit wird Airtime und Energie gespart. Entfernt sich der Node vom Gateway wird SNR und damit RSSI schlechter. Das System erhöht nun automatisch  den SF um eine bessere Übertragung sicherzustellen.\\[0.3cm]
Der Einsatz von ADR macht im ATAS System nur wenig Sinn. Dafür gibt es zwei Gründe:
\begin{itemize}
	\item Die ADR Kommunikation generiert einen Overhead.\\ Die Kommunikation benötigt jeweils zwei Uplink und eine Downlink Message. 
...
Die Verbreitung von LoRaWan in der Schweiz ist mittlerweile sehr hoch. Unternehmen wie die Swisscom und die Schweizer Post haben begonnen grosse Netzwerke aufzubauen \cite{swisscompost}.\\[0.3cm]
...
\end{figure}
\noindent
Violett steht für Abeckung. Weisse Stellen bedeuten es existiert keine LoRaWan Abdeckung. Wie in der Grafik ersichtlich, sind die städtischen Gebiete bereits erschlossen. Anders sieht es in den Bergregionen aus. Das Netzwerk der Swisscom und der Post kann somit für das ATAS System nicht verwendet werden.\
...
Durch die Wahl von TTN als LoRaWan Netzwerk muss die Datenübertragungsart MQTT genutzt werden. Der Zugriff auf die Datenschnittstelle (API) von TTN ist nur per MQTT möglich. Die Unterstützung von AMQP ist geplant aber noch nicht verfügbar \cite{amqp}. Das Protokoll MQTT war dem Projektteam bereits bekannt. Während dem Studium konnten bereits positive Erfahrungen mit MQTT gesammelt werden.

\chapter{Systembeschreibung - Detail}
Die vorhergegangene Architekturbeschreibung gibt einen groben ersten Überblick über das System. Technische Details zu  den verwendeten Technologien oder den genauen Fluss der Kommunikation waren nicht ersichtlich. In diesem Abschnitt soll nun intensiv auf diese Thematik eingegangen werden.\\[0.3cm]
Auf der nachfolgenden Seite finden Sie ein Schema, welches einen genauen Überblick über das System liefert.\\[0.3cm]
Blaue Boxen sind Komponenten. Um einen möglichst guten und kompletten Überblick über das System zu liefern, wird nicht zwischen Hard und Software unterschieden. Gelbe Wolken markieren einen Internetservice welcher für den Aufbau des Systems integriert wurde. Blaue Zylinder markieren eine Datenbanklösung. Das Abbild einer Person markiert einen User des Systems.
...
\begin{itemize}
	\item
	Alarm Schalter: Mit dem Druck auf den Knopf können die Überwacher über eine Notsituation aufmerksam gemacht werden. Bspw. Wenn der Alpinist einen Unfall hatte und nun nicht mehr mobil ist.
	\item
...
Der ATAS-Service verwaltet die erfassten Gefahrenzonen. Er empfängt die Geo-Koordinaten der Tracker und vergleicht diese mit den Koordinaten der Gefahrenzonen. Wenn sich ein Tracker in einer Gefahrenzone befindet, wird ein Alarmsignal an den Tracker gesendet. Das Signal wird mittels MQTT an CloudMQTT gesandt. Das TTN Netzwerk wird informiert und übernimmt die Zustellung der Nachricht an den Tracker.\\[0.3cm]
ATAS-Service bietet eine REST Schnittstelle an. Informationen zu den Gefahrenzonen können so von der WebApplikation (ATAS-Web) bezogen und gespeichert werden. Um die Daten langfristig zu speichern wurde eine MongoDB als Datenbanklösung installiert und konfiguriert.\\[0.3cm]
...
\end{figure}
\chapter{Prototyp}
Die Bachelorarbeit wird in 2 Hauptaufgaben aufgeteilt.\textbf{ Testing} und das Erstellen eines zweiten Prototyps nachfolgend \textbf{Prototyp 2} genannt. Dieses Kapitel beschreibt, wie beim Entwickeln des Prototypen 2 vorgegangen wurde.
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item Durch die Sicherheitslücken in einem OS, müssen regelmässig Patches eingespielt werden.
	\end{itemize}
...
Aus meiner Sicht bildet der während der Arbeit zu konstruierende Prototyp 2 nur einen Zwischenschritt bis zur Finalen Version. Eine Visualisierung meiner Idee:\\
\begin{figure}[H]
	\centering
...
Auf Grundlage des bestehenden Prototyps soll neue Hardware evaluiert werden. Der Funktionsumfang des Prototyps soll gleichwertig bleiben. Sind die Komponenten erst definiert, wird das Material bestellt.\\[0.3cm]
...
\begin{tabularx}{\linewidth}{XXX}
		...
		GPS Modul& ublox Neo 6M & \parbox[c]{1em}{
...
\begin{itemize}
	...
	Während der Arbeit soll die Kommunikation zwischen dem Tracker und dem Gateway getestet werden.
...
\begin{tabularx}{\textwidth}{ l|X }
	...
	Mittlere Distanz zum Gateway & 12Km \\ \hline
\end{tabularx}
...
sudo ./install.sh spi
\end{lstlisting}
Alles in allem eine sehr einfache Prozedur die nicht viel Zeit in Anspruch genommen hat. Anhand der Mac Adresse des Raspberry PI wird im Abschluss des Skripts eine \gls{DevEUI} generiert.
\newpage
...
Das ATAS System soll geprüft werden, ob es praxistauglich ist. Um dies zu überprüfen, werden in der vorher definierten Testregion 2, Messungen durchgeführt. Es soll getestet werden, wie zuverlässig die Übertragung funktioniert.\\[0.3cm]
...
\begin{figure}[H]
	...
	{Test 1 Resultate}
\end{figure}
Die erste Messung wurde mit einem Abstand von 5m durchgeführt. Diese Messung soll als Referenzwert dienen.
\newpage
\subsubsection{Auswertung}
Die Resultate des ersten Tests decken etwas sehr interessantes auf. Die Distanz zwischen Gateway und Tracker hat einen \textbf{kleinen Einfluss} auf die Übertragungsqualität. Wie wir vom letzten Datenpunkt ablesen können, ist es auch mit einer Distanz von über 7Km möglich, ein Signal sauber zu übertragen. Mehr Einfluss haben die Hindernisse zwischen Sender und Empfänger. Ist ein Hindernis bspw. ein Hügel im Weg, wird die Übertragung massiv gestört.
...
\end{figure}
\noindent
Auch bei SF12 liegt die Zeit zwischen den Übertragungen bei nur 170 Sekunden. Das bedeutet, dass wir ca. alle 3 Minuten den neue Standort übermitteln können. Beim Wandern wird diese Rate ausreichen. Ob diese Zeit für Skifahrer reicht, müssten in weiteren Tests festgestellt werden. Würden wir, wie bereits angesprochen, eine eigen ADR Lösung implementieren könnten wir den SF auch reduzieren. Dies würde und ermöglichen schneller zu übertragen.

\newpage
\subsection{Ergebnisse}
Aus den gewonnen Resultaten lässt sich bereits ableiten, ob das ATAS System praxistauglich ist. Beim ersten Tests haben wir festgestellt, dass sobald sich ein Hindernis bspw. ein Hügel oder viele Bäume zwischen Tracker und Gateway befindet, die Übertragung sehr stark gestört wird. Befinden wir uns auf der Gateway abgewendeten Talseite, kann teilweise kein Signal mehr zum Gateway durchdringen. Würden wir nun auf der anderen Seite des Berges einen weiteren Gateway installieren wäre das Problem gelöst, die Übertragung würde wieder funktionieren. Die Form eines Berges erinnert zwar von der Seite betrachtet meist an ein Dreieck, in der Realität sieht sie aber oft anders aus. Die Berge sind eine sehr dynamische Landschaft. Ein bildliches Beispiel:
\begin{figure}[H]
	\centering
...
%\newpage
%\subsubsection{Test 4: Wetterbedingungen}
%\textbf{Ausgangslage}: 
...
\chapter{Anhang}
\section{Projekt 2}
Das Vorprojekt 'Projekt 2 - ATAS', dieser Arbeit:\\[0.3cm] \url{https://github.com/schmm2/atas-doc/raw/master/Projekt2_ATAS_Schmidli.pdf}
\section{Software}
...
\newglossaryentry{DevEUI}{name={DevEUI},description={eindeutige Identifikationsnummer, 64bit}}
\newglossaryentry{ADR}{name={ADR},description={LoRaWan Endgerät selbständig ob und wie die Kommunikation optimiert werden soll}}
